...
Original file is located at
    https://colab.research.google.com/drive/1IoB9hIR-RDiUI73OZQwNDVgHoVwhO7tE
"""
...
def parse_top_block(soup):
    """Парсит верхние реквизиты через form-group label -> input[value]."""
    out = {}
    top_fields_map = {
...
def parse_panel_tables(soup):
    """
    Универсально парсит все panel.panel-default на вкладке general.
    Каждую таблицу превращает в словарь th -> td.
    """
    out = {}
    ...
    for panel in panels:
        ...
        # определяем префикс по названию секции
        if "Общие сведения" in heading:
            prefix = "Общие_"
        elif "Информация об организаторе" in heading:
            prefix = "Организатор_"
        else:
            # другие секции сейчас можно тоже забирать на будущее
        ...
        for tr in table.select("tr"):
            ...
            # если внутри td список li — собираем список
            ...
            # пропускаем дубли сумм/лотности, если хочешь
...
async def fetch_detail_page(session, tender, max_retries=3):
    ...
    # иногда стабильнее явно указывать general
...
async def run_detail_scraper(all_tenders_data):
    # лучше поменьше параллельность для goszakup
    ...
    async def limited_fetch(session, tender):
        async with sem:
            # микропаузa чтобы не бить ровным потоком
...
print("Сохранено: goszakup_tenders_full_async.csv")
print(df.columns)